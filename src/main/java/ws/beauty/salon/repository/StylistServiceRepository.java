...
public interface StylistServiceRepository extends JpaRepository<StylistService, StylistServiceId> {
    ...
    @Query("SELECT ss FROM StylistService ss " + "LEFT JOIN FETCH ss.stylist " + "LEFT JOIN FETCH ss.service " + "LEFT JOIN FETCH ss.appointment " + "WHERE ss.id.appointmentId = :appointmentId " +
       "AND ss.id.serviceId = :serviceId " + "AND ss.id.stylistId = :stylistId")
    ...
    @Query("SELECT DISTINCT ss FROM StylistService ss " + "LEFT JOIN FETCH ss.stylist " + "LEFT JOIN FETCH ss.service " + "LEFT JOIN FETCH ss.appointment")
